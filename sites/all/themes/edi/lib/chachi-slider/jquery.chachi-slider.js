    ...
    createSlider: function() {
      ...
      function eachSlide(i, slide) {
        ...
        if ($slide.find('img').attr('src')) {
            ...
            caption = '<div class="highlighted-text"><h2>' + $slide.find('img').attr('alt') + '</h2><a href="node/1705" class="go-to-map-btn">SEE THE MAP</a></div>',
            photoAuthor = $slide.find('.photo-author')

            captionHtml = captionHtml.append(caption, photoAuthor);

          slideHtml
            .css('background-image', 'url(' + $slide.find('img').attr('src') + ')')
            .append(captionHtml);
        } else {
